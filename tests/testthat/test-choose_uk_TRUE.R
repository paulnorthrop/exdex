...
k_vals <- 1:3
cres <- choose_uk(newlyn, u = u, k = k_vals, inc_cens = TRUE)
n_u <- length(u)
comp <- function(i, j) {
  return((i - 1) * n_u + j)
}
for (i in 1:length(k_vals)) {
  for (j in 1:length(u)) {
    res <- kgaps(newlyn, u[j], k_vals[i], inc_cens = TRUE)
...
imt_theta <- choose_uk(sp500, u = u, k = 1:5, inc_cens = TRUE)
...
u <- quantile(sp500, probs = seq(0.1, 0.9, by = 0.1))
imt_theta <- choose_uk(sp500, u = u, k = 1, inc_cens = TRUE)
ukplot <- plot(imt_theta)
test_that("plot.choose_uk works", {
  testthat::expect_identical(ukplot, NULL)
})
